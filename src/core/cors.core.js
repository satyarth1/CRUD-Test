...
const cores = async (ctx, next) => {
  const defaultOptions = {
    ...
    credentials: false,
    maxAge: 86400,
    allowHeaders: ['Content-Type', 'Authorization', 'Accept', 'Referer', 'User-Agent', 'x-token', 'x-content-type'],
  };
  ...
  if (defaultOptions.exposeHeaders) {
    ctx.set('Access-Control-Expose-Headers', defaultOptions.exposeHeaders.join(','));
  }
